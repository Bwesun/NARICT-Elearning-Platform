	...
	<form method="post">
      ...
      ?>
			...
			<div class="col-md-9">
				...
				<h2><?php echo $rows['course_title'] ?> Course</h2>
				<p><?php echo $rows['course_description']  ?></p>
				
				<fieldset>
					<legend>Add Course Content</legend>
				...
				if(isset($_POST['item_sub'])){
					...
					$sql="INSERT INTO course_contents(course_id, content_title, content, file_type)VALUES('$course_id','$content_title', '$filename', '$file_type') ";
					$result=mysql_query($sql);

					if($result){
						echo "<script>alert('Content Added Successfully!')</script>";
						//echo $sql;
						echo "<script>window.open('View_course.php?id=".$collected_id."', '_self')</script>";
					}else{
				...
				<fieldset>
					...
					<?php
								if(isset($_POST['sub_delete_item'])){
									...
									$sql3="DELETE FROM course_contents WHERE id='$item_id' ";
